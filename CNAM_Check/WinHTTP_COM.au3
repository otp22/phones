...
Global Const $_WinHTTP_LibVer='1.9'
Global $_WinHTTP_Debug=False
...
Func _WinHTTP_Startup()
	...
	If $_WinHTTP_UseDefaultErrorHandler Then
		If Not IsObj($_WinHTTP_oErrorHandler) Then
			$_WinHTTP_Stage='Event ErrorHandler'
...
Func _WinHTTP_Shutdown()
	Global $_WinHTTP_oErrorHandler,$_WinHTTP_oRequest,$_WinHTTP_Error,$_WinHTTP_StayEnabled
...
Func _WinHTTP_Request($method='GET',$URL='http://www.example.com/',$Content='',$File='',$aHeaders=0)
	If $_WinHTTP_Debug Then ConsoleWrite('WinHTTP - GET '&$URL&' '&$Content&@CRLF)
	Global $_WinHTTP_Error
	$_WinHTTP_Error=0
	...
	If Not IsObj($_WinHTTP_oRequest) Then Return '';And IsObj($_WinHTTP_oAdodb))
